 ...
 * Requires manage_user_activation. When activating (is_active: true), sends notification email if Resend is configured.
 */
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    const auth = await requireAuthAndGlobalPermission(request, "manage_user_activation");
