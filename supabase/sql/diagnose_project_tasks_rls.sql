...
-- 3) Check membership for a specific project (RLS uses pm.user_id = auth.uid() only)
...
SELECT pm.id, pm.project_id, pm.user_id, pm.role,
       ...
       (pm.user_id = auth.uid()) AS membership_check_passes
...
-- 5) Optional: list projects you are a member of (RLS uses pm.user_id = auth.uid() only)
SELECT p.id AS project_id, p.name,
       pm.user_id, pm.role,
       (pm.user_id = auth.uid()) AS i_am_member
...
-- RLS uses user_id only. Replace <PROJECT_UUID> with the project id, then run:
/*
